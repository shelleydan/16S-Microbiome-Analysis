...
ord_pcoa_uf <- ordinate(PHYLOSEQ_OBJ, method = "NMDS", distance = "unifrac", weighted = TRUE)

stress_val <- ord_pcoa_uf$stres

ord_plot <- plot_ordination(PHYLOSEQ_OBJ, ord_pcoa_uf, color = "Groups") +
  stat_ellipse(type = "t", linetype = 2, level = 0.95) +
  ggtitle(sprintf("NMDS (Weighted UniFrac) — Stress = %.3f", stress_val))
...
ord_nmds <- ordinate(PHYLOSEQ_OBJ, method = "NMDS", distance = "bray", trymax = 200)
ord_plot <- plot_ordination(PHYLOSEQ_OBJ, ord_nmds, color = "Sample_Name") +
  stat_ellipse(type = "t", linetype = 2, level = 0.68) + # optional group ellipse
  geom_point(size = 3) + theme_bw() + labs(title = "NMDS (Bray–Curtis)")
ggplotly(ord_plot)
```
...
#    pcoa = stats::cmdscale(unif, k=3, eig=T)
...
# 
#  adonis_result <- adonis2(adonis_dist ~ type, data = meta_df, by = "margin", permutations = 99)
...
```{r}
# ggplotly(p4, width = 1800, height = 1000)
```
...
# 
# 
# x_lab <- paste0(method, " 1 (", format(100 * eig[1] / sum(eig), digits = 4), "%)")
# y_lab <- paste0(method, " 2 (", format(100 * eig[2] / sum(eig), digits = 4), "%)")
# z_lab <- paste0(method, " 3 (", format(100 * eig[3] / sum(eig), digits = 4), "%)")
# 
# 
# plot_ly(
#   data = points,
...
#   y = ~y,
#   z = ~Z,
#   color = ~Sample_Name,   # or whatever metadata column you want
#   type = "scatter3d",
#   mode = "markers",
#   width = 1800,
#   height = 1000
...
```{r}
 ...
 scale_size_continuous(range = c(1, 20)) +
  ...
  strip.height = unit(10, "cm")   # ← Increase this to make more space
)
...
```{r plotting_taxonomy, fig.width=10, fig.height=20}
ggplotly(p4, width = 1800, height = 1000)
```
